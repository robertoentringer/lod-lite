...
const fill = async () => {
  if (args.resource) {
    update('name', args.name || path.basename(args.resource, path.extname(args.resource)))
  } else {
    update('resource', await opendata.url)
    update('name', args.name || (await opendata.slug))
  }
