...
const { status } = require('./logger')

const start = (entry) => {
  const id = sequential(entry, schema.meta.id).toString()
  const version = sequential(entry, schema.meta.version).toString()

  logger.info('Lod version: %s', version)
  logger.spinner(id)
  ...
  status.entries[id] = item

  if (Object.keys(status.entries).length === args.max) logger.clear() & end()
}
...
const end = () => {
  ...
  logger.info('Entries extracted: %s', Object.keys(status.entries).length)
...
const main = async () => {
  ...
  resource(args.resource, start, end)
}

module.exports = { main, start, end }
