...
const end = () => {
  ...
  logger.info('Extracted files : %s', logger.status.files.length)
  logger.info('Write fails : %s', logger.status.fails.length, util.inspect(logger.status.fails))
  logger.warn('Execution time: %s', logger.time())
  logger.exit()
}
